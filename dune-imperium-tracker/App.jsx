...
function App() {
  ...
  const handleGameAction = (actionDetails) => {
    ...
    if (actionDetails.action === 'VP_ADJUST') {
      ...
      const targetPlayer = gameData.players.find(p => p.name === actionDetails.playerName);
      const vpBefore = targetPlayer ? (targetPlayer.vp || 0) : 0;
      ...
      // Enhanced Log Entry for VP Logs
      const logEntry = {
        ...actionDetails,
        round: currentRound, // Inject Round
        // Keep raw details for analytics
        rawDetails: {
          category,
          actionName,
          points,
