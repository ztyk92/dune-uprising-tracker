...
export default function VPTrackingView({ players, alliances, round = 1, vpActions, onAction, onRoundChange, onUndo, onEndGame }) {
    ...
    return (
        <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '1000px', margin: '0 auto', boxSizing: 'border-box' }}>
            {/* Round Number module */}
            <div style={{
                display: 'flex',
                alignItems: 'center',
            ...
            {/* Player Points Tracker module (0-14 horizontal tiling) */}
            <div style={{
                display: 'grid',
                ...
                {Array.from({ length: TRACK_SPACES }, (_, i) => i).map(space => {
                    ...
                    return (
                        <div key={space} style={{
                            ...
                            alignItems: 'center',
                            justifyContent: 'center',
                            backgroundColor: isGolden ? 'rgba(212, 175, 55, 0.15)' : '#222',
                            border: isGolden ? '1px solid rgba(212, 175, 55, 0.4)' : '1px solid #444',
                            borderRadius: '4px',
                            minHeight: '80px',
                            ...
                            <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: '2px', alignItems: 'center', width: '100%' }}>
                                {playersHere.map(p => {
                                    const col = COLOUR_MAP[p.colour] || { bg: '#888', glow: '#aaa' };
                                    return (
                                        <div key={p.name} title={`${p.name}: ${p.vp || 0} VP`} style={{
                                            ...
                                            borderRadius: '4px',
                                            backgroundColor: col.bg,
                                            boxShadow: `0 0 4px ${col.glow}aa, inset 0 2px 4px rgba(255,255,255,0.3)`,
                                            border: `1px solid ${col.glow}`,
                                            flexShrink: 0,
                                        }} />
                                    );
